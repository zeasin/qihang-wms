...
import cn.qihangerp.api.common.TaoApiCommon;
...
public class TaoRefundApiController {
    ...
    @ResponseBody
    public AjaxResult refundOrderPull(@RequestBody PullRequest taoRequest) throws IOException {
...
//        return new ApiResult<>(EnumResultVo.SUCCESS.getIndex(), msg);
    ...
    @ResponseBody
    public AjaxResult refundDetailPull(@RequestBody PullRequest taoRequest) throws IOException {
        log.info("/**************主动更新tao退货订单****************/");
        if (taoRequest.getShopId() == null || taoRequest.getShopId() <= 0) {
            return AjaxResult.error(HttpStatus.PARAMS_ERROR, "参数错误，没有店铺Id");
        }
